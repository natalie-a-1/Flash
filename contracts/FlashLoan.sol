...
contract FlashLoan is FlashLoanSimpleReceiverBase {
    ...
    ) external override returns (bool) { // Restore override
        // 1. Input Validation
    ...
    function withdrawToken(address _tokenAddress) external onlyOwner {
        if (_tokenAddress != USDC && _tokenAddress != WETH) revert InvalidAsset(_tokenAddress, _tokenAddress); // Use InvalidAsset for simplicity or add a new error
    ...
    function getUniswapRouterAddress() external view returns (address) { return address(uniswapRouter); }

    /** @notice Gets the configured SushiSwap V2 Router address. */
    function getSushiSwapRouterAddress() external view returns (address) { return address(sushiRouter); }
    ...
    function _getMinAmountOut(
        ...
        try IUniswapV2Router02(_router).getAmountsOut(_amountIn, _path) returns (uint[] memory _expectedAmounts) {
    ...
    function _safeApprove(address _token, address _spender, uint256 _amount) internal {
        ...
        (bool success, bytes memory data) = _token.call(abi.encodeWithSelector(IERC20.approve.selector, _spender, _amount));
