...
contract SushiSwapInteractor is Ownable {
    ...
    function swapExactTokensForTokens(
        ...
        if (_tokenIn == address(0) || _tokenOut == address(0) || _to == address(0)) revert InvalidTokenAddress();
        if (_amountIn == 0) revert ZeroAmount();
        ...
        try sushiRouter.swapExactTokensForTokens(
            ...
            if (keccak256(abi.encodePacked(reason)) == keccak256(abi.encodePacked("UniswapV2Router: EXPIRED"))) {
                 revert ExpiredDeadline();
             } else if (keccak256(abi.encodePacked(reason)) == keccak256(abi.encodePacked("UniswapV2Router: INSUFFICIENT_OUTPUT_AMOUNT"))) {
