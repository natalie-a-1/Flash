...
function checkAndCompileContracts() {
  ...
  if (!fs.existsSync(buildDir) || fs.readdirSync(buildDir).length === 0) {
    console.log('📝 Contracts not compiled or build directory is empty. Compiling now...');
    try {
...
function startGanacheFork() {
  ...
  if (!mainnetRpcUrl) {
    console.error('❌ NEXT_PUBLIC_MAINNET_RPC_URL environment variable is required');
    process.exit(1);
  }
  ...
  const ganacheArgs = [
    '--fork', `${mainnetRpcUrl}@${blockNumber}`,
    '--unlock', whaleAddress,
    '-a', '10',
    '-e', '1000',
    '-d',
...
function deployAndApprove() {
  ...
  const web3Path = path.join(__dirname, '..', 'test', 'demo', 'checkContract.js');
  ...
  try {
    ...
    console.log('✅ Contract already exists, proceeding with router approvals...');
    runRouterApprovals();
  } catch (error) {
    // Contract doesn't exist, deploy it first
    console.log('⚠️ Contract not found at the expected address, deploying a new contract...');
    const deployScript = path.join(__dirname, '..', 'test', 'demo', 'deployFlashLoan.js');
...
function runRouterApprovals() {
  ...
  const approveScript = path.join(__dirname, '..', 'test', 'demo', 'approveRouters.js');
  ...
  nodeProcess.on('exit', (code) => {
    if (code === 0) {
      ...
      console.log('⚠️ Ganache will continue running, but routers may not be approved');
