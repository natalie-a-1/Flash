...
const WETH_ADDRESS = process.env.SEPOLIA_WETH_ADDRESS || "0xfFf9976782d46CC05630D1f6eBAb18b2324d6B14"; // Example Sepolia WETH
const USDC_ADDRESS = process.env.SEPOLIA_USDC_ADDRESS || "0x94a9D9AC8a22534E3FaCa9F4e7F2E2cf85d5E4C8"; // Example Sepolia USDC (6 decimals)
...
contract("UniswapInteractor (Forked Sepolia)", (accounts) => {
    ...
    before(async () => {
        ...
        console.log(`   Funding account ${fundingAccount} initial ETH balance: ${web3.utils.fromWei(initialEthBalance)} ETH`);
    ...
    beforeEach(async () => {
        ...
        try {
            ...
            console.log(`   [BeforeEach] Funding account WETH balance: ${web3.utils.fromWei(currentWethBalance)}`);

            if (currentWethBalance.lt(amountToFundWeth)) {
                const wrapAmount = amountToFundWeth.sub(currentWethBalance);
                console.log(`   [BeforeEach] Insufficient WETH. Wrapping ${web3.utils.fromWei(wrapAmount)} ETH from ${fundingAccount} into WETH...`);

                const ethBalance = await web3.eth.getBalance(fundingAccount);
                if (new BN(ethBalance).lt(wrapAmount)) {
                     throw new Error(`Funding account ${fundingAccount} has insufficient ETH (${web3.utils.fromWei(ethBalance)}) to wrap ${web3.utils.fromWei(wrapAmount)} WETH.`);
                 }

                await weth.deposit({ from: fundingAccount, value: wrapAmount });
                currentWethBalance = await weth.balanceOf(fundingAccount);
                console.log(`   [BeforeEach] New Funding account WETH balance: ${web3.utils.fromWei(currentWethBalance)}`);
                assert(currentWethBalance.gte(amountToFundWeth), "WETH balance is still insufficient after wrapping.");
            }
        } catch (e) {
            console.error(`   [BeforeEach] ERROR during WETH balance check/wrap. Error: ${e.message}`);
        ...
        try {
            ...
            console.log(`   [BeforeEach] Funded interactor ${interactorInstance.address} with ${web3.utils.fromWei(amountToFundWeth)} WETH`);
            assert(new BN(balanceInteractorAfter).sub(new BN(balanceInteractorBefore)).eq(amountToFundWeth), "Interactor WETH funding transfer failed");
        } catch (e) {
            const currentWethBalance = await weth.balanceOf(fundingAccount);
            console.error(`   [BeforeEach] ERROR: Failed to fund interactor instance. Funding account WETH: ${web3.utils.fromWei(currentWethBalance)}. Error: ${e.message}`);
            throw e;
        }
    ...
    it("should have correct Uniswap Router address set", async () => {
        ...
        assert.equal(router.toLowerCase(), UNISWAP_ROUTER_ADDRESS.toLowerCase(), "Router address mismatch");
    });

    it("should retrieve amounts out for WETH -> USDC", async () => {
        const amounts = await interactorInstance.getAmountsOut(WETH_ADDRESS, USDC_ADDRESS, pointOneWeth);
        assert.isArray(amounts, "Amounts should be an array");
        assert.equal(amounts.length, 2, "Amounts array should have 2 elements");
        assert.equal(amounts[0].toString(), pointOneWeth.toString(), "Input amount in array mismatch");
        console.log(`   0.1 WETH -> ${amounts[1].div(oneUsdc).toString()} USDC (approx)`);
    ...
    it("should swap WETH for USDC successfully", async () => {
        ...
        assert(new BN(swapEvent.amounts[1]).gte(amountOutMin), "Event output lower than calculated min");
        ...
        console.log(`   Swapped ${web3.utils.fromWei(amountIn)} WETH for ${usdcReceived.div(oneUsdc).toString()} USDC`);
        assert(usdcReceived.gte(amountOutMin), "Received USDC less than specified minimum");
        assert(new BN(interactorWethBalanceBefore).sub(new BN(interactorWethBalanceAfter)).eq(amountIn), "Interactor WETH balance did not decrease correctly");
    });
    ...
    it("should revert swap if deadline expired", async () => {
        ...
        await truffleAssert.reverts(
            interactorInstance.swapExactTokensForTokens(
                WETH_ADDRESS, USDC_ADDRESS, amountIn, amountOutMin, recipient, deadline, { from: deployer }
            ),
        ...
        await truffleAssert.reverts(
            interactorInstance.swapExactTokensForTokens(
                "0x0000000000000000000000000000000000000000", // Zero address
                 USDC_ADDRESS, amountIn, amountOutMin, recipient, deadline, { from: deployer }
            ),
