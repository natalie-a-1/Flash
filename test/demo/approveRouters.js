...
function findContractArtifact() {
  ...
  console.error(
    "❌ ERROR: Could not find appropriate contract artifact with router approval methods.",
  );
  process.exit(1);
}
...
async function findDeployedContractAddress(FlashLoanArtifact) {
  ...
  if (envAddress) {
    console.log(
      `📌 Using contract address from environment variable: ${envAddress}`,
    );
    return envAddress;
  }
  ...
  try {
    ...
    if (deployedNetwork && deployedNetwork.address) {
      console.log(
        `📌 Found contract address from deployment artifacts: ${deployedNetwork.address}`,
      );
      return deployedNetwork.address;
    }
  } catch (error) {
    console.error(
      "⚠️ Error getting network or checking artifact:",
      error.message,
    );
  }
...
async function testContractConnection(contractInstance) {
  ...
  try {
    ...
    console.log(
      `✅ Contract code exists at ${contractInstance.options.address}`,
    );
    return true;
  } catch (error) {
...
async function verifyContractMethods(contractInstance) {
  ...
  for (const method of requiredMethods) {
    if (!methods[method]) {
      console.error(
        `❌ ERROR: Required method '${method}' not found on contract`,
      );
      return false;
    }
  }
...
async function testRouterApproval(contractInstance, routerAddress) {
  try {
    console.log(
      `🧪 Testing approvedRouters(${routerAddress.substring(0, 10)}...) method...`,
    );
    const isApproved = await contractInstance.methods
      .approvedRouters(routerAddress)
      .call();
    ...
    try {
      ...
      const isApproved = await contractInstance.methods
        .isRouterApproved(routerAddress)
        .call();
      console.log(
        `✅ isRouterApproved() call succeeded! Approved: ${isApproved}`,
      );
      return true;
    } catch (altError) {
      console.error(
        "❌ ERROR: isRouterApproved() call also failed:",
        altError.message,
      );
      return false;
    }
...
async function approveRouters() {
  try {
    ...
    const contractAddress =
      await findDeployedContractAddress(FlashLoanArtifact);
    ...
    if (!connectionValid) {
      console.error(
        "❌ ERROR: Cannot proceed due to contract connection issues.",
      );
      return false;
    }
    ...
    if (!methodsValid) {
      console.error(
        "❌ ERROR: Cannot proceed due to missing contract methods.",
      );
      return false;
    }
    ...
    if (!contractOwner) {
      console.error(
        "❌ ERROR: Cannot retrieve contract owner, approvals will not work.",
      );
      return false;
    }
    ...
    if (contractOwner.toLowerCase() !== deployerAccount.toLowerCase()) {
      console.error(
        `❌ ERROR: The account ${deployerAccount} is not the contract owner!`,
      );
      console.error(
        `❌ Only the contract owner ${contractOwner} can approve routers.`,
      );
      console.error(
        "❌ Make sure the contract deployer account is available in your Ganache instance.",
      );

      // Try to suggest an account to use
      if (
        accounts.some(
          (acc) => acc.toLowerCase() === contractOwner.toLowerCase(),
        )
      ) {
        ...
        console.log(
          `💡 TIP: Use account #${ownerIndex} (${contractOwner}) for approval instead.`,
        );
      } else {
        console.log(
          `💡 TIP: Add the owner address to Ganache's unlocked accounts.`,
        );
      }
    ...
    const canCallApproval = await testRouterApproval(
      flashLoanContract,
      UNISWAP_V2_ROUTER,
    );
    if (!canCallApproval) {
    ...
    try {
      const uniApproved =
        await flashLoanContract.methods[methodToUse](UNISWAP_V2_ROUTER).call();
      const sushiApproved =
        await flashLoanContract.methods[methodToUse](
          SUSHISWAP_V2_ROUTER,
        ).call();

      console.log(
        `📝 Current status: Uniswap V2 Router approved: ${uniApproved}, SushiSwap Router approved: ${sushiApproved}`,
      );

      // Approve routers if needed
      if (!uniApproved) {
        console.log("🔄 Approving Uniswap V2 Router...");
        const tx = await flashLoanContract.methods
          .setRouterApproval(UNISWAP_V2_ROUTER, true)
          .send({ from: deployerAccount, gas: 200000 });
        console.log(
          "✅ Uniswap V2 Router approved successfully!",
          tx.transactionHash,
        );
      } else {
      ...
      if (!sushiApproved) {
        ...
        const tx = await flashLoanContract.methods
          .setRouterApproval(SUSHISWAP_V2_ROUTER, true)
          .send({ from: deployerAccount, gas: 200000 });
        console.log(
          "✅ SushiSwap Router approved successfully!",
          tx.transactionHash,
        );
      } else {
      ...
      const uniApprovedAfter =
        await flashLoanContract.methods[methodToUse](UNISWAP_V2_ROUTER).call();
      const sushiApprovedAfter =
        await flashLoanContract.methods[methodToUse](
          SUSHISWAP_V2_ROUTER,
        ).call();
      ...
      console.error(
        `❌ ERROR: Failed to perform router approvals:`,
        error.message,
      );
      return false;
    }
