...
function getFlashLoanArtifact() {
  ...
  if (!fs.existsSync(flashLoanPath)) {
    console.error('❌ ERROR: FlashLoan.json not found. Please compile the contracts first.');
    process.exit(1);
  }
...
async function deployFlashLoan() {
  try {
    ...
    console.log(`📦 Deploying FlashLoan contract with Aave Addresses Provider: ${AAVE_ADDRESSES_PROVIDER}`);
    ...
    if (deployedCode === '0x' || deployedCode === '0x0') {
      console.error('❌ ERROR: Contract deployment verification failed. No code at address.');
    ...
    try {
      ...
      if (owner.toLowerCase() !== deployerAccount.toLowerCase()) {
        console.warn(`⚠️ WARNING: Owner (${owner}) is not the deployer (${deployerAccount})`);
      }
    } catch (error) {
    ...
    try {
      ...
      await deployedContract.methods.setRouterApproval(uniswapRouter, true)
      ...
      await deployedContract.methods.setRouterApproval(sushiswapRouter, true)
        .send({ from: deployerAccount, gas: 200000 });
