...
async function skewPrices() {
  ...
  const skewAmountUSDC = web3.utils.toBN('50000').mul(web3.utils.toBN('1000000')); // 50,000 USDC
  const pathUsdcToWeth = [USDC_ADDRESS, WETH_ADDRESS];
  const deadline = Math.floor(Date.now() / 1000) + 60 * 10; // 10 minutes

  // Check balance before swap
  const myUsdcBalance_bn = web3.utils.toBN(await usdc.methods.balanceOf(me).call());
  console.log(`My current USDC balance: ${web3.utils.fromWei(myUsdcBalance_bn, 'mwei')} USDC`);

  if (myUsdcBalance_bn.lt(skewAmountUSDC)) {
    throw new Error(`Insufficient USDC balance to perform skew. Need ${web3.utils.fromWei(skewAmountUSDC, 'mwei')}, have ${web3.utils.fromWei(myUsdcBalance_bn, 'mwei')}. Run seed script first?`);
  }

  console.log(`Executing Uniswap swap with ${web3.utils.fromWei(skewAmountUSDC, 'mwei')} USDC...`);
  try {
    ...
    console.log(`Swapped ${web3.utils.fromWei(skewAmountUSDC, 'mwei')} USDC → WETH on Uniswap`);
  } catch (swapError) {
  ...
  try {
    ...
    console.log('--- Final Calculated Pool Prices (WETH per 1 USDC, scaled 1e18) ---');
    ...
    if (uniPriceFromRouter.gt(sushiPriceFromRouter)) {
      console.log('✅ Price successfully skewed! (Uniswap WETH/USDC > SushiSwap WETH/USDC)');
    } else {
      console.warn('⚠️ Price skew might not be significant or might be reversed.');
    }
  } catch (priceError) {
