...
async function seedWallet() {
  ...
  console.log(
    `Whale initial USDC balance: ${web3.utils.fromWei(initialWhaleBalance, "mwei")} USDC`,
  );
  ...
  console.log(
    `Attempting to transfer ${web3.utils.fromWei(seedAmount, "mwei")} USDC from whale to ${me}...`,
  );
  await usdc.methods
    .transfer(me, seedAmount.toString())
  ...
  console.log(
    `My new USDC balance: ${web3.utils.fromWei(myUsdcBalance, "mwei")} USDC`,
  );
  ...
  const maxApproval = web3.utils
    .toBN("2")
    .pow(web3.utils.toBN("256"))
    .sub(web3.utils.toBN("1"))
    .toString(); // Max uint256

  console.log(
    `Approving Uniswap Router (${UNISWAP_ROUTER_ADDR}) for max USDC...`,
  );
  await usdc.methods
    .approve(UNISWAP_ROUTER_ADDR, maxApproval)
    .send({ from: me, gas: 100000, gasPrice: fixedGasPrice });
  console.log("Approved Uniswap Router.");

  console.log(
    `Approving Sushiswap Router (${SUSHISWAP_ROUTER_ADDR}) for max USDC...`,
  );
  await usdc.methods
    .approve(SUSHISWAP_ROUTER_ADDR, maxApproval)
    .send({ from: me, gas: 100000, gasPrice: fixedGasPrice });
