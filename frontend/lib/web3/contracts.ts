 ...
 */
export async function loadContract(contractName: string): Promise<Web3Contract | null> {
  try {
    ...
    if (!contractJson.networks[networkId]) {
      console.warn(`Contract ${contractName} not deployed on network ${networkId}`);
      return null;
    }
    
    // Create and return the contract instance
    return createContractInstance(contractJson.abi, contractJson.networks[networkId].address);
  } catch (error) {
 ...
 */
export function createContractInstance(abi: any[], address: string): Web3Contract {
