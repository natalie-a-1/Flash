  ...
  return (
    <div className="fixed bottom-4 right-4 z-50 bg-slate-800/60 backdrop-blur px-2 py-1.5 rounded-lg border border-slate-700/50 shadow-md">
      <div className="flex items-center text-xs">
        {isLoading ? (
          <svg className="w-3 h-3 mr-1.5 text-blue-400 animate-spin" fill="none" viewBox="0 0 24 24">
            <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
            <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
          </svg>
        ) : (
          <div className={`w-2 h-2 rounded-full mr-1.5 ${lastUpdated ? 'bg-green-500/80' : 'bg-amber-500/80'}`}></div>
        )}
